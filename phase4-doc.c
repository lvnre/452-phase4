...
//initialize proc table and populate the appropriate elements of the systemCallVec array to point to system call functions
void
start3(void)
{
  char	name[128];
...
//process requests delay for a specified number of seconds and checks on each clock interrupt to see which process(es) to wake
int
...
//Processesuser requests to diskWrite, diskRead, and diskSize
int
DiskDriver(char *arg)
{
...
//Handles user requests to termRead and termWrite
...
//Receives a line of input from a mailbox associated with the indicated terminal
...
//Sends a line of output to the indicated terminal
...
//This helper function for diskRead populates the necessary system arguments to 
...
//This function specifies that the disk read function was requested by the user
...
//This helper function for diskWrite populates the necessary system arguments to 
//carry out the operation
...
//This function specifies that the disk write function was requested by the user
...
//This function handles actually performing either the disk read or disk write operation
...
//This helper function for diskSizeReal populates the necessary system arguments to 
//carry out the operation
...
//This function handles actually performing the read Disk Size operation
int diskSizeReal(int unit, int *sector, int *track, int *disk) {
...
//This helper function for termReadReal populates the necessary system arguments to 
//carry out the operation
...
//This function handles actually performing the terminal read operation
...
//This helper function for termWriteReal populates the necessary system arguments to 
//carry out the operation
...
//This function handles actually performing the terminal write operation
...
//initialize an entry in the process table
...
//returns the member at the top of the sleeping queue
procStructPtr topSleepingQ(pQueue *q)
{
  return &q->processes[0];
}

//appends a member to the sleeping queue
...
//removes the top member of the sleeping process queue
procStructPtr removeTopSleepingQ(pQueue *q)
{
...
//appends a member to the specified disk list
...
//Removes the last member of the specified disk list
procStructPtr removeFromDiskList(diskList *list)
{
  if(list->length == 0)
    return NULL;
